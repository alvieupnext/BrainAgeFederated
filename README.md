...
- **`Mixture` (Approximated IID Setup)**: Fits a two-component Gaussian Mixture Model (GMM) to the entire dataset to mathematically learn the "shape" and distribution of the global population. It then populates all nodes equally by sampling from this synthetic, continuous probability density function. This provides an IID baseline similar to `Original`, but analytically smooths out noisy outliers and discrete anomalies found in the raw dataset.
